...
export default function ProductCard({ product }: { product: Product }) {
  ...
  function handleAdd() {
    ...
    const key = [product.id, variantId, Object.keys(extrasOn).filter((k) => extrasOn[k]).join("+"), product.hasFlavor ? flavor : "", colors.join("-"), product.hasShape ? shape : ""]
      .filter(Boolean)
      .join("::");
    ...
    <div className="flex flex-col overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-olive-200/60 transition hover:-translate-y-1 hover:shadow-lg">
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-gradient-to-br from-olive-100 to-blush-100">
        {product.image ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-olive-400 paw-bg">
            ...
            <span className="font-display text-sm">Foto próximamente</span>
          </div>
        )}
        {product.tagline && (
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-2">
          <div className="font-display text-xl font-bold text-blush-500">
