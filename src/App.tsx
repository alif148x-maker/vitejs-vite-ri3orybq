...
export default function CroquisApp() {
  ...
  useEffect(() => {
    ...
    const selectionRing = new THREE.Mesh(new THREE.RingGeometry(0.01, 0.02, 32), new THREE.MeshBasicMaterial({ color: store.primary_color, side: THREE.DoubleSide, transparent: true, opacity: 0.9 }));
    ...
    const onUp = (e) => { if (dragRef.current.dragging && dragRef.current.moved < 6) doRaycastSelect(e); dragRef.current.dragging = false; };
  ...
  const waMessage = useMemo(() => {
    ...
    return encodeURIComponent(`Hola ${store.name}, quiero cotizar este espacio:\n\n${lines.join("\n")}\n\nTotal aprox: $${total.toLocaleString()}\nCuarto: ${room.width}×${room.length} m`);
  ...
  if (loadStatus === "error" || !store) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center p-6" style={{ background: "#F5F2EC", fontFamily: "Inter, sans-serif" }}>
        No se pudo cargar la tienda "{STORE_SLUG}". Revisa que exista en la tabla <code>stores</code> y que las políticas RLS permitan lectura pública.
      </div>
    );
  }
  ...
  return (
    <div className="croquis-root">
      <style>{`
        ...
        .croquis-root{ --bg:#F5F2EC; --ink:#211D18; --brand:${store.primary_color}; --brand-light:#7C948B; --walnut:#96602F; --line:#DED7C7; --panel:#FCFAF6;
          font-family:'Inter',sans-serif; color:var(--ink); background:var(--bg); min-height:100vh; display:flex; flex-direction:column; }
        ...
        .tabpill{ font-family:'IBM Plex Mono',monospace; font-size:11px; padding:5px 10px; border-radius:999px; border:1px solid var(--line); white-space:nowrap; }
        .tabpill.active{ background:var(--brand); color:#fff; border-color:var(--brand); }
        .catcard{ border:1px solid var(--line); background:var(--panel); }
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b" style={{ borderColor: "var(--line)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="disp font-bold text-base">{store.name}</div>
        </div>
        <span className="mono text-[10px] px-2 py-1 rounded-full border" style={{ borderColor: "var(--line)", color: "var(--walnut)" }}>CONECTADO A SUPABASE</span>
      </header>
      <main className="flex-1 flex flex-col lg:flex-row gap-4 p-4 sm:p-6 max-w-[1400px] w-full mx-auto">
        <div className="lg:w-[280px] flex-shrink-0 flex flex-col gap-4 order-1">
          <div className="cardline rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3"><Ruler size={16} style={{ color: "var(--brand)" }} /><h3 className="disp font-semibold text-sm">Dimensiones</h3></div>
            <div className="grid grid-cols-3 gap-2 mb-3">
              {[{ key: "width", label: "Ancho" }, { key: "length", label: "Largo" }, { key: "height", label: "Altura" }].map((f) => (
                <label key={f.key} className="text-[11px] flex flex-col gap-1">
                  ...
                  <input type="number" step="0.1" value={room[f.key]} onChange={(e) => setRoomField(f.key, Number(e.target.value) || 0)} className="mono w-full border rounded px-1.5 py-1 text-sm" style={{ borderColor: "var(--line)" }} />
                </label>
              ))}
            </div>
            <div className="mb-3">
              <div className="text-[11px] mb-1.5 flex items-center gap-1" style={{ color: "var(--brand-light)" }}><Palette size={12} /> Pared</div>
              <div className="flex gap-1.5 flex-wrap">{WALL_COLORS.map((c) => <button key={c.hex} className={`swatch ${room.wallColor === c.hex ? "active" : ""}`} style={{ background: c.hex }} onClick={() => setRoomField("wallColor", c.hex)} />)}</div>
            </div>
            <div>
              <div className="text-[11px] mb-1.5 flex items-center gap-1" style={{ color: "var(--brand-light)" }}><Home size={12} /> Piso</div>
              <div className="flex gap-1.5 flex-wrap">{FLOOR_COLORS.map((c) => <button key={c.hex} className={`swatch ${room.floorColor === c.hex ? "active" : ""}`} style={{ background: c.hex }} onClick={() => setRoomField("floorColor", c.hex)} />)}</div>
            </div>
          </div>
          {selectedItem && selectedCat && (
            <div className="cardline rounded-xl p-4">
              ...
              <button onClick={removeSelected} className="w-full flex items-center justify-center gap-1.5 text-[12px] py-1.5 rounded border" style={{ borderColor: "#B0472F55", color: "#B0472F" }}><Trash2 size={13} /> Quitar</button>
            </div>
          )}
        </div>
        <div className="flex-1 flex flex-col gap-2 order-2 min-w-0">
          <div className="relative rounded-xl overflow-hidden cardline" style={{ height: 420 }}>
            ...
            <div className="absolute top-2 right-2 flex flex-col gap-1.5">
              ...
              <button onClick={topView} className="ctrlbtn rounded-lg p-2 shadow-sm"><Compass size={14} /></button>
              <button onClick={resetView} className="ctrlbtn rounded-lg p-2 shadow-sm mono text-[9px]">RST</button>
            </div>
          </div>
          <div className="cardline rounded-xl p-4 flex-1 flex flex-col min-h-0">
            <div className="flex items-center gap-2 mb-2.5"><ShoppingCart size={16} style={{ color: "var(--brand)" }} /><h3 className="disp font-semibold text-sm">Catálogo</h3></div>
            ...
            <div className="grid sm:grid-cols-2 gap-2.5 overflow-y-auto pr-1" style={{ maxHeight: 260 }}>
              ...
              {filteredCatalog.length === 0 && <p className="text-[12px] col-span-2 py-4 text-center" style={{ color: "var(--brand-light)" }}>Este catálogo aún no tiene productos.</p>}
            </div>
          </div>
        </div>
      </main>
